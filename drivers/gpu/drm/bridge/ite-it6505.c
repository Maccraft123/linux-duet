...
static bool it6505_get_sink_hpd_status(struct it6505 *it6505)
{
	...
	return (reg_0d & HPD_STS) ? true : false;
}
...
{
	if (it6505_read(it6505, 0xCF) == 0xF0)
		it6505_set_bits(it6505, REG_DRV_LN_DATA_SEL, 0x80, 0x00);

	if (it6505_read(it6505, 0xCF) == 0x70) {
		if (it6505->lane_swap) {
...
{
	it6505_set_bits(
		it6505, REG_LINK_DRV, 0xF1,
		((BIT(it6505->lane_count) - 1)
		 << (it6505->lane_swap ? (8 - it6505->lane_count) : 4)) |
			0x01);
}
...
static bool it6505_reset_logic(struct it6505 *it6505)
{
	regmap_write(it6505->regmap, REG_RESET_CTRL, ALL_LOGIC_RESET);
	usleep_range(1000, 1500);

	it6505->revision = it6505_read(it6505, REG_IC_VER);

	if (it6505->revision < 0)
		return false;
...
aux_op_start:
	...
	if (cmd == CMD_AUX_I2C_EDID_READ) {
		/* DP AUX EDID FIFO has maximum length of n bytes. */
	...
	if (cmd == CMD_AUX_I2C_EDID_READ) {
		for (i = 0; i < size; i++) {
			status = it6505_read(it6505, REG_AUX_DATA_FIFO);
			if (status < 0)
				goto aux_op_err;
			buffer[i] = status;
		}
		...
		for (i = 0; i < size; i++) {
			status = it6505_read(it6505, REG_AUX_DATA_0_7 + i);
			if (status < 0)
				goto aux_op_err;
...
static void it6505_update_video_prarmeter(struct it6505 *it6505,
					  const struct drm_display_mode *mode)
{
...
static void it6505_parse_link_capabilities(struct it6505 *it6505)
{
	...
	DRM_DEV_DEBUG_DRIVER(dev, "Sink max link rate: %d.%d Gbps per lane",
...
static bool it6505_check_max_value(enum max_value_check type,
				   u8 lane_voltage_swing_pre_emphasis)
{
	if (type == VOLTAGE_SWING)
		return ((lane_voltage_swing_pre_emphasis & 0x03) ==
			MAX_CR_LEVEL);
	else
		return ((lane_voltage_swing_pre_emphasis & 0x03) ==
			MAX_EQ_LEVEL);
...
static bool it6505_get_video_status(struct it6505 *it6505)
{
	...
	return (reg_0d & VIDEO_STB) ? true : false;
}
...
static void it6505_link_training_work(struct work_struct *work)
{
	...
	it6505_aux_reset(it6505);
	err = it6505_link_start_auto_train(it6505);
	DRM_DEV_DEBUG_DRIVER(dev, "auto train %s, auto_train_retry: %d",
			     err ? "pass" : "failed", it6505->auto_train_retry);
...
step_train:
	...
	for (i = 0; i < step_retry; i++) {
		...
		it6505_dpcd_write(it6505, DP_TRAINING_PATTERN_SET,
				  DP_TRAINING_PATTERN_DISABLE);
		err = it6505_link_start_step_train(it6505);
...
static int it6505_procss_hpd_irq(struct it6505 *it6505)
{
	struct device *dev = &it6505->client->dev;
	int ret, dpcd_sink_count, dp_irq_vector;
	...
	/* no need to check link status if no training */
...
static void it6505_check_reg06(struct it6505 *it6505, unsigned int reg06)
{
	...
	if (reg06 & INT_HPD_CHANGE) {
		...
		if (data) {
			 ...
			 * when sink counnt change from  0->1
	...
	if (reg06 & INT_RECEIVE_HPD_IRQ) {
		...
		if (it6505_procss_hpd_irq(it6505) < 0)
...
static int it6505_get_modes(struct drm_connector *connector)
{
	...
	for (i = 0; i < 3; i++) {
		...
		it6505->edid =
		drm_do_get_edid(&it6505->connector, it6505_get_edid_block,
				it6505);
				   ...
				   const struct drm_display_mode *adjusted_mode)
{
	...
	it6505_update_video_prarmeter(it6505, adjusted_mode);
...
static void it6505_parse_dt(struct it6505 *it6505)
{
	...
	it6505->lane_swap_disabled = true;
	it6505->afe_setting = 0;
	...
	it6505->afe_setting = afe_setting;
}
